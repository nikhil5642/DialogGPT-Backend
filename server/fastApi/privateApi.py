...
from server.fastApi.modules.firebase_verification import (
    generate_JWT_Token,
    get_current_user,
    verifyFirebaseLogin,
)
from server.fastApi.modules.stripeSubscriptionMangement import (
...
origins = ["http://localhost:3000", "https://www.dialoggpt.io", "https://dialoggpt.io"]
...
def createBot(
    data: ChatBotCreationModel, current_user: str = Depends(get_current_user)
):
    try:
...
def myChatbots(current_user: str = Depends(get_current_user)):
    try:
        chatbot_list, chatbot_limit, is_first_time_user = getUserChatBotInfo(
            current_user
        )
        return {
            SUCCESS: True,
...
def myChatbotInfo(
    data: BaseChatBotModel, current_user: str = Depends(get_current_user)
):
    try:
...
async def fetchURLs(data: URLListModel, current_user: str = Depends(get_current_user)):
    try:
        ...
        return {
            SUCCESS: True,
            RESULT: get_filtered_content_mapping(current_user, data.botID, mapping),
        }
    except:
...
async def updateURLDataAsync(data: TrainingModel, current_user: str):
    ...
    filtered_mapping = get_filtered_content_mapping(
        current_user, data.botID, new_url_mapping
    )
...
async def updateURLData(
    data: TrainingModel, current_user: str = Depends(get_current_user)
):
...
def reply(reply: ReplyModel):
    try:
        ...
        if model[MODEL_VERSION] == GPT_3_5_TURBO and msgCredits > 0:
            chat_reply = replyToQuery(
                model, reply.botID, reply.query, reply.history[-5:]
            )
        ...
        history.append(
            ChatMessageModel(id=len(history) + 1, text=reply.query, type="outgoing")
        )
        history.append(
            ChatMessageModel(id=len(history) + 1, text=chat_reply, type="incoming")
        )
...
def updateName(
    data: ChatBotNameChangeModel, current_user: str = Depends(get_current_user)
):
    try:
...
def updateChatbotModel(
    data: ChatBotModelModel, current_user: str = Depends(get_current_user)
):
    try:
        updateChatModel(
            current_user, data.botID, data.prompt, data.modelVersion, data.temperature
        )
        return {SUCCESS: True}
    except:
        raise HTTPException(status_code=501, detail="Something went wrong, Try Again!")
...
def updateChatBotInterface(
    data: ChatBotLeadsModel, current_user: str = Depends(get_current_user)
):
    try:
...
def updateChatBotInterface(
    data: ChatBotInterfaceModel, current_user: str = Depends(get_current_user)
):
    try:
        updateChatInterface(
            current_user,
