...
import stripe
from server.fastApi.modules.databaseManagement import get_subscription_id, get_subscription_plan, handle_subscription_creation, handle_subscription_deletion, handle_subscription_update, to_camel_case
from src.DataBaseConstants import BASIC_PLAN, CUSTOMER, FREE_PLAN, STATUS, SUBSCRIPTION, SUBSCRIPTION_ID,USER_ID,PLAN_ID,PRO_PLAN,ESSENTIALS_PLAN,ID
import requests
...
def createStripeCheckoutSession(user_id,plan_id):
    ...
    # print(price_id,old_subscription_id)
...
def create_new_subscription_checkout_session(user_id,plan_id,price_id):
    subscription_data= {"trial_period_days": 7} if plan_id == 'BASIC_PLAN' else {}
...
def manageWebhook(payload,sig_header):
    ...
    if event.type == 'checkout.session.completed':
        ...
        if USER_ID in data.metadata:
            handle_subscription_update(data.metadata[USER_ID],data[STATUS],data.metadata[PLAN_ID], getMessageLimitAsPerPlan(data.metadata[PLAN_ID]))
