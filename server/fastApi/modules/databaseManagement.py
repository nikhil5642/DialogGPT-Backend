...
def createChatBot(uid:str,chatbotname):
    ...
    if(len(bot_id_list)>=max_allowed_bots):
        raise HTTPException(status_code=400, detail="You have reached the maximum limit of chatbots for your plan")
...
def setLeadsCollection(uid:str,botID:str,leadCollection:[str]):
    if not getChatBotConfigCollection().find_one({USER_ID:uid,CHATBOT_ID:botID}):
         getChatBotConfigCollection().insert_one({USER_ID:uid,CHATBOT_ID:botID,LEADS_COLLECTION:leadCollection})
    else:
        getChatBotConfigCollection().update_one({USER_ID: uid, CHATBOT_ID: botID}, {"$set": {LEADS_COLLECTION: leadCollection}})
...
def storeLead(chatId,leadName,leadEmail,leadPhone):
    ...
    if existing_history:
        getChatHistoryCollection().update_one(
            {CHAT_ID: chatId},
            {"$set": {LEAD_NAME: leadName,LEAD_EMAIL:leadEmail,LEAD_PHONE:leadPhone,LAST_UPDATED:datetime.now()}}
        )
    else:
        getChatHistoryCollection().insert_one({LEAD_NAME: leadName,LEAD_EMAIL:leadEmail,LEAD_PHONE:leadPhone,LAST_UPDATED:datetime.now()})
...
def getChatInterface(botID:str):
    ...
    if interface_data == {}:
        return {INITIAL_MESSAGE:BASE_CHAT_INITIAL_MESSAGE,QUICK_PROMPTS:"",THEME:LIGHT,PROFILE_PICTURE:None,USER_MSG_COLOR:BASE_USER_MSG_COLOR,DISPLAY_NAME:"",CHAT_ICON:None,CHAT_BUBBLE_COLOR:BASE_CHAT_BUBBLE_COLOR,LEADS_COLLECTION:leads_data}
    else:
        return {INITIAL_MESSAGE:interface_data[INITIAL_MESSAGE] or BASE_CHAT_INITIAL_MESSAGE,QUICK_PROMPTS:interface_data[QUICK_PROMPTS],THEME:interface_data[THEME],PROFILE_PICTURE:interface_data[PROFILE_PICTURE],USER_MSG_COLOR:interface_data[USER_MSG_COLOR],DISPLAY_NAME:interface_data[DISPLAY_NAME],CHAT_ICON:interface_data[CHAT_ICON],CHAT_BUBBLE_COLOR:interface_data[CHAT_BUBBLE_COLOR],LEADS_COLLECTION:leads_data}    
