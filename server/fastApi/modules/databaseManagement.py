...
from DataBase.MongoDB import (
    getChatBotConfigCollection,
    getChatHistoryCollection,
    getContentStoreCollection,
    getUsersCollection,
    getChatBotsCollection,
)
from src.BaseConstants import (
    BASE_CHAT_BUBBLE_COLOR,
    BASE_CHAT_INITIAL_MESSAGE,
    BASE_CHATGPT_PROMPT,
    BASE_USER_MSG_COLOR,
)
...
def getUserInfo(uid):
    return (
        getUsersCollection().find_one({USER_ID: uid}, {"_id": 0, CHATBOT_LIST: 0}) or {}
    )
...
def getMessageCredits(userDoc):
    if userDoc != {}:
        return {
            SUBSCRIPTION_PLAN: userDoc.get(SUBSCRIPTION_PLAN, FREE_PLAN),
            MESSAGE_CREDITS: max(
                0, userDoc.get(MESSAGE_LIMIT, 0) - userDoc.get(MESSAGE_USED, 0)
            ),
        }
    else:
    ...
    bot_id_list.append(
        {
            ...
            CHATBOT_STATUS: "untrained",
            LAST_UPDATED: datetime.now(),
            CREATED_ON: datetime.now(),
        }
    )
    getUsersCollection().update_one(
        {USER_ID: uid}, {"$set": {CHATBOT_LIST: bot_id_list}}
    )
    getChatBotsCollection().insert_one(
        {
            USER_ID: uid,
            CHATBOT_ID: botID,
            CHATBOT_NAME: chatbotname,
...
def updateChatBotStatus(uid, botID, status):
    if status == TRAINED:
        getChatBotsCollection().update_one(
            {USER_ID: uid, CHATBOT_ID: botID},
            {"$set": {CHATBOT_STATUS: status, LAST_UPDATED: datetime.now()}},
        )
    else:
        getChatBotsCollection().update_one(
            {USER_ID: uid, CHATBOT_ID: botID}, {"$set": {CHATBOT_STATUS: status}}
        )
    ...
    getUsersCollection().update_one(
        {USER_ID: uid}, {"$set": {CHATBOT_LIST: bot_id_list, IS_FIRST_TIME_USER: False}}
    )
...
def updateChatbotName(uid, botID, newName):
    ...
    getChatBotsCollection().update_one(
        {USER_ID: uid, CHATBOT_ID: botID}, {"$set": {CHATBOT_NAME: newName}}
    )
...
def getUserChatBotInfo(uid: str):
    ...
    if user is None:
        return []
    return (
        user.get(CHATBOT_LIST, []),
        getChatBotLimitAsPerPlan(user.get(SUBSCRIPTION_PLAN, FREE_PLAN)),
        user.get(IS_FIRST_TIME_USER, False),
    )
...
def getContentMappingList(uid: str, botID: str):
    ...
    try:
        return (
            getChatBotsCollection().find_one({USER_ID: uid, CHATBOT_ID: botID})[
                CONTENT_LIST
            ]
            or []
        )
...
def getChatBotInfo(uid: str, botID: str):
    return (
        getChatBotsCollection().find_one(
            {USER_ID: uid, CHATBOT_ID: botID},
            {"_id": 0, USER_ID: 0, CHATBOT_ID: 0, CONTENT_LIST: 0},
        )
        or {}
    )
...
def updateContent(uid: str, botID: str, contentID: str, content: str):
    ...
    getContentStoreCollection().update_one(
        {CONTENT_ID: contentID},
        {"$set": {CONTENT: content, LAST_UPDATED: datetime.now()}},
    )
...
def insertContentListInBotCollection(uid: str, botID: str, newContentList):
    getChatBotsCollection().update_one(
        {USER_ID: uid, CHATBOT_ID: botID}, {"$set": {CONTENT_LIST: newContentList}}
    )


def getChatLeadCollection(botID: str):
    document = getChatBotConfigCollection().find_one({CHATBOT_ID: botID})
    if document:
        leads_data = document.get(LEADS_COLLECTION, [])
    else:
        leads_data = []
...
def setLeadsCollection(uid: str, botID: str, leadCollection: [str]):
    if not getChatBotConfigCollection().find_one({USER_ID: uid, CHATBOT_ID: botID}):
        getChatBotConfigCollection().insert_one(
            {USER_ID: uid, CHATBOT_ID: botID, LEADS_COLLECTION: leadCollection}
        )
    else:
        getChatBotConfigCollection().update_one(
            {USER_ID: uid, CHATBOT_ID: botID},
            {"$set": {LEADS_COLLECTION: leadCollection}},
        )


def storeLead(chatId, leadName, leadEmail, leadPhone):
...
def getChatInterface(botID: str):
    ...
    if interface_data == {}:
        return {
            INITIAL_MESSAGE: BASE_CHAT_INITIAL_MESSAGE,
            QUICK_PROMPTS: "",
            THEME: LIGHT,
            PROFILE_PICTURE: None,
            USER_MSG_COLOR: BASE_USER_MSG_COLOR,
            DISPLAY_NAME: "",
            CHAT_ICON: None,
            CHAT_BUBBLE_COLOR: BASE_CHAT_BUBBLE_COLOR,
            LEADS_COLLECTION: leads_data,
        }
    else:
        return {
            INITIAL_MESSAGE: interface_data[INITIAL_MESSAGE]
            or BASE_CHAT_INITIAL_MESSAGE,
            QUICK_PROMPTS: interface_data[QUICK_PROMPTS],
            THEME: interface_data[THEME],
            PROFILE_PICTURE: interface_data[PROFILE_PICTURE],
            USER_MSG_COLOR: interface_data[USER_MSG_COLOR],
            DISPLAY_NAME: interface_data[DISPLAY_NAME],
            CHAT_ICON: interface_data[CHAT_ICON],
            CHAT_BUBBLE_COLOR: interface_data[CHAT_BUBBLE_COLOR],
            LEADS_COLLECTION: leads_data,
        }


def updateChatInterface(
    uid: str,
    botID: str,
    initialMessage: str,
    quickPrompts: str,
    ...
    if not getChatBotConfigCollection().find_one({USER_ID: uid, CHATBOT_ID: botID}):
        getChatBotConfigCollection().insert_one(
            {USER_ID: uid, CHATBOT_ID: botID, INTERFACE: interface_data}
        )
    else:
        getChatBotConfigCollection().update_one(
            {USER_ID: uid, CHATBOT_ID: botID}, {"$set": {INTERFACE: interface_data}}
        )
...
def getChatModel(botID: str):
    ...
    if model_data == {}:
        return {
            PROMPT: BASE_CHATGPT_PROMPT,
            BASE_PROMPT: BASE_CHATGPT_PROMPT,
            MODEL_VERSION: GPT_3_5_TURBO,
            TEMPERATURE: 0,
        }
    else:
        return {
            PROMPT: model_data[PROMPT] or BASE_CHATGPT_PROMPT,
            BASE_PROMPT: BASE_CHATGPT_PROMPT,
...
def updateChatModel(
    uid: str, botID: str, prompt: str, modelVersion: str, temperature: float
):
    model_data = {PROMPT: prompt, MODEL_VERSION: modelVersion, TEMPERATURE: temperature}

    if not getChatBotConfigCollection().find_one({USER_ID: uid, CHATBOT_ID: botID}):
        getChatBotConfigCollection().insert_one(
            {USER_ID: uid, CHATBOT_ID: botID, MODEL: model_data}
        )
    else:
        getChatBotConfigCollection().update_one(
            {USER_ID: uid, CHATBOT_ID: botID}, {"$set": {MODEL: model_data}}
        )
...
def deleteChatbot(uid, botID):
    ...
    getChatBotConfigCollection().delete_one({USER_ID: uid, CHATBOT_ID: botID})
    getChatHistoryCollection().delete_many({CHATBOT_ID: botID})
