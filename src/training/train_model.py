...
from server.fastApi.modules.awsKeysManagement import get_openai_api_key
...
def getDocumentsList(contentIDList):
    ...
    for item in contentIDList:
        docList.append(
            Document(
                page_content=contentMapping[item[CONTENT_ID]],
                metadata={SOURCE: item[SOURCE], SOURCE_TYPE: item[SOURCE_TYPE]},
            )
        )
    return docList
